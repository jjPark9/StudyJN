...
1. Date(날짜) 객체 </br>
   날짜와 시간을 관리하는 객체 </br>
   특별한 날짜를 기준으로 생성하기를 원한다면 인수에 원하는 날짜를 입력하면 됨

   ```
   var 이름 = new Date();
   var 이름 = new Date('년도/월/일');
   var 이름 = new Date(년도, 월-1, 일);
   ```

   1. get 메소드 그룹 </br>
      대부분 서버의 날짜 정보를 가져와 사용

      |      메소드       | 설명                                          |
      | :---------------: | --------------------------------------------- |
      |     getDate()     | 1부터 31일까지 해당 달의 일을 가져옴          |
      |     getDay()      | 요일을 숫자로 가져옴 (0:일요일 ~ 6:토요일)    |
      |   getFullYear()   | 년도를 4자리 수로 가져옴                      |
      |    getHours()     | 시간의 정보를 가져옴 (0-23)                   |
      |   getMinutes()    | 분의 정보를 가져옴 (0-59)                     |
      |    getMonth()     | 월의 정보를 숫자로 가져옴 (0:1월 ~ 11:12월)   |
      |   getSeconds()    | 초의 정보를 가져옴 (0-59)                     |
      | getMilliseconds() | 1초를 밀리초로 반환하여 0~999의 숫자로 가져옴 |
      |     getTime()     | 1970년 1월 1일 이후의 시간을 밀리초로 표시    |

   1. set 메소드 그룹 </br>
      사용자의 필요 값이 설정될 때 </br>
      생성된 날짜 인스턴스에 날짜 정보를 설정할 때 사용

      |      메소드       | 설명                                                 |
      | :---------------: | ---------------------------------------------------- |
      |     setDate()     | 1-31의 숫자로 일을 지정                              |
      |   setFullYear()   | 4자리 숫자로 연도 지정 (옵션으로 월, 일도 지정 가능) |
      |    setHours()     | 0-23의 숫자로 시간 지정                              |
      | setMilliseconds() | 밀리초를 지정                                        |
      |    setMonth()     | 0-11의 숫자로 월을 지정                              |
      |   setSeconds()    | 0-59의 숫자로 초를 지정                              |
      |     setTime()     | 1970년 1월 1일 이후의 시간을 밀리초로 설정           |
      | toLocaleString()  | YY/MM/DD hh:mm:ss 형태로 시간과 날짜를 문자로 지정   |
      |    toSource()     | Date() 객체의 원형을 반환                            |
      |    toString()     | 시간과 날짜를 문자열로 반환                          |

1. Math(수학) 객체 </br>
   수학과 관련된 기능과 속성 제공 </br>

   1. 메소드
      | 메소드 | 설명 |
      | :----: | ---- |
      | abs(number) | 절댓값 반환 |
      | ceil(number) | 소수점 부분 올림 |
      | floor(number) | 소수점 부분 버림 |
      | round(number) | 소수점 부분을 반올림 |
      | random() | 0과 1 사이의 난수를 반환 |
      | max(number, number) | 여러 숫자 중 큰 수를 반환 |
      | min(number, number) | 여러 숫자 중 작은 수를 반환 |

# 문서 객체 모델 (DOM)

HTML 문서의 여러 요소를 자유롭게 이동하며 원하는 작업을 할 수 있는 시스템

1. 노드 </br>
   HTML 문서를 구성하고 있는 개별적인 단위 </br>
   DOM은 노드의 집합임

   - 노드의 종류
     - 요소 노드(element node)
     - 텍스트 노드(texxt node)
     - 속성 노드(attribute node) </br></br>

## DOM 요소 선택

|               메소드               | 설명                     |
| :--------------------------------: | ------------------------ |
|    getElementsById('아이디명')     | 아이디명으로 요소 선택   |
|   getElementsByTagName('태그명')   | 태그명으로 요소 선택     |
| getElementsByClassName('클래스명') | 클래스명으로 요소 선택   |
|     querySelector('css선택자')     | css선택자로 요소 선택    |
|   querySelectorAll('css선택자')    | 같은 css선택자 모두 선택 |

> 여러 개의 요소가 있을 경우에는 배열로 선택됨
...
1. css 선택자
   ...
   | 후손 선택자 | 선택자1 선택자2 | 선택자1 아래에 있는 선택자2를 선택 |

   [기타 선택자 모음](https://lcs1245.tistory.com/entry/JavaScriptJQuery-%EC%84%A0%ED%83%9D%EC%9E%90-Selector-%EB%AA%A8%EC%9D%8C)
   ...
   ***
   ...
   <th>설명</th>
   <tr>
      <td rowspan=2>innerHTML</td>
      <td>선택자.innerHTML</td>
      <td>선택된 요소 안의 노드를 가져옴</td>
   </tr>
   <tr>
      <td>선택자.innerHTML = 값</td>
      <td>선택된 요소 안의 노드값을 변경</td>
   </tr>
   <tr>
      <td>outerHTML</td>
      <td>선택자.outerHTML</td>
      <td>선택된 요소 밖의 노드를 가져옴</td>
   </tr>
   </table>
...
|  classList.remove('클래스명')  | 클래스명 삭제                                       |
|  classList.toggle('클래스명')  | 클래스명이 있는 경우에는 없애고, 없는 경우에는 추가 |
| classList.contains('클래스명') | 해당되는 클래스명이 있으면 true, 없으면 false       |

## DOM 속성 변경

|            메소드            | 설명                                        |
| :--------------------------: | ------------------------------------------- |
|    getAttribute('속성명')    | 선택된 속성 노드의 값을 가져옴              |
| setAttribute('속성명', '값') | 선택된 속성 노드의 값을 변경                |
|  removeAttribute('속성명')   | 선택된 속성 노드의 값을 삭제                |
|    hasAttribute('속성명')    | 해당되는 속성명이 있으면 true, 없으면 false |
...
| :----------------------: | ------------------------------------------ |
|     createElement()      | 요소 생성                                  |
|     createTextNode()     | text 콘텐츠 생성                           |
| appendChild(새로운 노드) | 새로 생성된 노드를 요소가 끝나기 전에 삽입 |
|         append()         | 새로 생성된 노드를 요소가 끝나기 전에 삽입 |
|     cloneNode(옵션)      | 노드 복사                                  |
